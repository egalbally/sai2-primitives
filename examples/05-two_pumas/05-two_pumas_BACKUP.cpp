...
void recordData(double curr_time, int dof, Eigen::Vector3d sensed_force1, Eigen::Vector3d sensed_moment1, Eigen::Vector3d sensed_force2, Eigen::Vector3d sensed_moment2);
...
int main (int argc, char** argv) {
	...
	auto fsensor = new ForceSensorSim(robot_name, ee_link_name, T_sensor, robot);
	...
	thread sim_thread(simulation, fsensor, sim);
...
void control(Sai2Model::Sai2Model* robot1, Sai2Model::Sai2Model* robot2, Sai2Model::Sai2Model* object_model, Simulation::Sai2Simulation* sim) {
	...
	while (fSimulationRunning) { //automatically set to false when simulation is quit
		...
		if (curr_time <= 8.0)
		{
			recordData(curr_time, dof, sensed_force1, sensed_moment1, sensed_force2, sensed_moment2);
		}
	}
}
...
void recordData(double curr_time, int dof, Eigen::Vector3d sensed_force1, Eigen::Vector3d sensed_moment1, Eigen::Vector3d sensed_force2, Eigen::Vector3d sensed_moment2) {
	...
	for (int i = 0; i < 3; i++) {
		data(1 + i) = sensed_force1(i);
		data(4 + i) = sensed_moment1(i);
	}
